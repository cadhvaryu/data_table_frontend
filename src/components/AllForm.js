...
class TemplateField extends React.Component {
	...
	constructor(props) {
      ...
      layoutRecords: [],
		...
		//this.addBlockModalToggle = this.addBlockModalToggle.bind(this);
	...
	addBlockModalToggle = (item) => {
		let success = {};
		const { fields } = this.state;
		fields['tbmBlockName'] = item.tbmBlockName;
		this.setState({ loading:false, blockId: item.tbmId, fields: fields, addBlockModal:!this.state.addBlockModal, success });
	}

	closeBlockModalToggle = () => {
		let success = {};
		const { fields } = this.state;
		fields['tbmBlockName'] = '';
		fields['tbmLayoutId'] = '';
		this.setState({ loading:false, blockId: '', fields: fields, addBlockModal:!this.state.addBlockModal, success });
	}
  ...
  checkBoxMandatoryChecked = (e) => {
	...
	updateBlock = () => {
		if (this.handleBlockValidation()) {	
			const { fields, blockId, templateId } = this.state;
			this.setState({ loading:true });
			let jsonObject = JSON.stringify({
        tbmBlockName: fields['tbmBlockName']
      });

			const url = API + 'templates/updateBlock/'+blockId;
      const headers = new Headers({
        ...jsonHeader,
      })
      const options = {
			...
			fetch(url, options)
			    ...
			    .then(data => {
			        ...
			        fields['tbmBlockName'] = '';
					    let success = {};
			        success["successMessage"] = data.message;
					    this.setState({ loading:false, fields : fields, addBlockModal:!this.state.addBlockModal, success });
                ...
                <option value="">Select Template</option>
                {
                  ...
                  templateRecords.map((templateRecord, i) => {
                    ...
                    <Row>
										...
										{
											(fields['tfmField'] === 'checkbox' || fields['tfmField'] === 'dropdown' || fields['tfmField'] === 'multiselect' || fields['tfmField'] === 'radio') ? (
												<Row>
													<Col>
														<FormGroup>
									...
									<CardBody>
										<Form className="brandForm" method="POST">
											...
											<Row>
												...
												<Col lg="4">
													<Button type="button" className="brandButton mb-4 mt-4" onClick={ this.addTemplateBlockToggle }>{"Close" }</Button>
													<Button type="button" className="brandButton mb-4 mt-4" onClick={ this.SubmitBlock }>{ "Save" }</Button>
												</Col>
											</Row>
										</Form>	
									</CardBody>
												...
												<Card body>
													<CardBody>
														<Row className="mt-1 mb-3">
															<Col className="d-flex align-items-center">
																	{
																		addBlockModal === true && item.tbmId === blockId ? (
																			<React.Fragment>
																				<Input type="text" style={{ width: "30%"}} name="tbmBlockName" id="tbmBlockName" onChange={this.handleChange.bind(this, "tbmBlockName")} value={fields['tbmBlockName']} />
																				<Button type="button" className="lgnButton ml-2 mr-2" onClick={() => this.closeBlockModalToggle()}>{"Close" }</Button>
																				<Button type="button" onClick={() => this.updateBlock()}  className="lgnButton">Save</Button>
																			</React.Fragment>
																		) : (
																			<React.Fragment>
																				<h4 className="m-0">{item.tbmBlockName}</h4>
																				<Button type="button" onClick={() => this.addBlockModalToggle(item)}  className="lgnButton ml-2"><i className="fa fa-edit" aria-hidden="true" title="Edit Custom Block"></i></Button>
																			</React.Fragment>
																		)
																	}
															</Col>
